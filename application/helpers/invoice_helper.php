 ...
 * @return string
 */
function invoice_logo( $logo = null )
{
    $CI = &get_instance();

    $logo = $logo ? $logo : $CI->mdl_settings->setting('invoice_logo');

    if ($CI->mdl_settings->setting('invoice_logo')) {
        return '<img  style="width:200px;" src="' . base_url() . 'uploads/' . $logo . '">';
    }
...
{
    return [
        "EUR","USD","GBP","RUB","AED","AUD","BGN","BRL","CAD","CHF","CLP","CNY","COP","CZK","DKK","EGP","HKD","HRK","HUF","IDR","ILS","INR","JPY","KRW","MXN","MYR","NOK","NZD","PLN","RON","SEK","SGD","THB","TRY","UAH","ZAR",
    ];
}
...
{
    $currency_symbols = [
        '$' => ['USD','CAD','CLP','COP','MXN','NZD','SGD','AUD'],
        'kr' => ["DKK","NOK","SEK"],
        '€' => "EUR",'£' => "GBP",'₽' => "RUB",'د.إ' => "AED",'лв' => "BGN",'R$' => "BRL",'CHF' => "CHF",'¥ /元' => "CNY",'Kč' => "CZK",'E£' => "EGP",'HK$' => "HKD",'kn' => "HRK",'ft' => "HUF",'Rp' => "IDR",'₪' => "ILS",'₹' => "INR",'¥' => "JPY",'₩' => "KRW",'RM' => "MYR",'zł' => "PLN",'lei' => "RON",'฿' => "THB",'₺' => "TRY",'₴' => "UAH",'R' => "ZAR",
    ];

    foreach ($currency_symbols as $s => $items){
        if( is_array($items) && in_array($currency,$items)){
            $symbol = $s;break;
        }
        elseif ($currency == $items ){
            $symbol = $s;break;
        }
    }
    return $symbol;
...
function get_converted_currency_rate( $from , $to )
{
    $url = file_get_contents('https://free.currencyconverterapi.com/api/v5/convert?q=' . $from . '_' . $to . '&compact=ultra&apiKey=99f83ee31042ed65517a');
