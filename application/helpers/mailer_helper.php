...
function email_ticket(
    ...
    $CI->load->helper('mailer/phpmailer');
    $CI->load->helper('template');
    $CI->load->model('tickets/mdl_tickets');

    $db_ticket = $CI->mdl_tickets->where('ip_tickets.ticket_id', $ticket_id)->get()->row();


    $message = parse_template($db_ticket, $body);
    $subject = parse_template($db_ticket, $subject);
    $cc = parse_template($db_ticket, $cc);
    $bcc = parse_template($db_ticket, $bcc);
    $from = array(parse_template($db_ticket, $from[0]), parse_template($db_ticket, $from[1]));

    $message = (empty($message) ? ' ' : $message);

    return phpmail_send($from, $to, $subject, $message, null, $cc, $bcc, $attachments);
}
...
function email_offer(
    ...
    $db_offer = $CI->mdl_offers->where('ip_offers.offer_id', $offer_id)->get()->row();

    $offer = generate_offer_pdf($offer_id, false, $offer_template);

    $message = parse_template($db_offer, $body);
    $subject = parse_template($db_offer, $subject);
    $cc = parse_template($db_offer, $cc);
    $bcc = parse_template($db_offer, $bcc);
    $from = array(parse_template($db_offer, $from[0]), parse_template($db_offer, $from[1]));

    $message = (empty($message) ? ' ' : $message);

    return phpmail_send($from, $to, $subject, $message, $offer, $cc, $bcc, $attachments);
}



function email_prospect(
    $from,
    $to,
    $subject,
    $body,
    ...
    $message = prospect_template($db_invoice, $body);


    $subject = prospect_template($db_invoice, $subject);
    $from = array(prospect_template($db_invoice, $from[0]), prospect_template($db_invoice, $from[1]));

    $message = (empty($message) ? ' ' : $message);

    return phpmail_send($from, $to, $subject, $message,  $attachments,'','','');
}
