...
<div class="container">
    ...
    <div id="content">
        ...
        <div class="quote">
            <?php
            if ($quote->quote_company_id != 0) { ?>
            <?php foreach ($companies as $company) : ?>
                <?php

                if ($company->id == $quote->quote_company_id) {
                    if ($logo = invoice_logo($company->logo)) {
                        echo $logo . '<br><br>';
                    }
                }
                ?>
            <?php endforeach ?>
           <?php } else{
                if ($logo = invoice_logo($companies[0]->logo)) {
                echo $logo . '<br><br>';
                }
             } ?>
            <br>
                    ...
                    <?php foreach ($companies as $company) : ?>
                        ...
                        if ($company->id == $quote->quote_company_id) { ?>
                            ...
                            <div class="col-xs-12 col-md-6 col-lg-5">
                                ...
                                <h4><?php _htmlsc($company->name); ?></h4>
                                <p><?php if ($company->vat_id) {
                                        echo lang("vat_id_short") . ": " . $company->vat_id . '<br>';
                                    } ?>
                                    <?php if ($company->street_1) {
                                        echo htmlsc($company->street_1) . '<br>';
                                    } ?>
                                    <?php if ($company->street_2) {
                                        echo htmlsc($company->street_2) . '<br>';
                                    } ?>
                                    <?php if ($company->city) {
                                        echo htmlsc($company->city) . ' ';
                                    } ?>
                                    <?php if ($quote->user_state) {
                                        echo htmlsc($quote->user_state) . ' ';
                                    } ?>
                                    <?php if ($company->zip_code) {
                                        echo htmlsc($company->zip_code) . '<br>';
                                    } ?>
                                    <?php if ($company->phone) { ?><?php echo trans('phone_abbr'); ?>: <?php echo htmlsc($company->phone); ?>
                                        <br><?php } ?>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered">
                        ...
                        <tr>
                            ...
                            <td class="amount"><?php echo format_currency($quote->quote_item_subtotal - ($quote->quote_item_subtotal / 100 * $quote->quote_discount_percent)); ?></td>
                        </tr>
                        ...
                        <?php if ($quote->quote_item_tax_total > 0) { ?>
                            <tr>
                                ...
                                <td class="amount"><?php echo format_currency($quote->quote_item_tax_total - ($quote->quote_item_tax_total / 100 * $quote->quote_discount_percent)); ?></td>
                            </tr>
                        <?php } ?>
