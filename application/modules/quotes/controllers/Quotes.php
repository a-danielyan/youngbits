 ...
 * Class Quotes
 */
class Quotes extends Custom_Controller
{
    /**
     * Quotes constructor.
     */
    ...
    public function status($status = 'all', $page = 0)
    {
        ...
        $success = [TYPE_FREELANCERS,TYPE_EMPLOYEES,TYPE_ADMINISTRATOR];
        ...
        if ($this->session->userdata('user_type') == TYPE_MANAGERS) {
            $this->mdl_quotes->TYPE_MANAGER_IS_GROUP($this->session->userdata('user_group_id'));
        }
    ...
    public function view($quote_id)
    {
        ...
        $access=[TYPE_MANAGERS,TYPE_ADMIN];
    ...
    public function generate_multi_pdf($quote_ids, $stream = true, $quote_template = null)
    {
        $quote_ids = explode('_', $quote_ids);
        $this->load->helper('pdf');

        if (get_setting('mark_quotes_sent_pdf') == 1) {
            $this->mdl_quotes->mark_sent($quote_ids);
        }

        generate_multi_quote_pdf($quote_ids, $stream, $quote_template);
    }
