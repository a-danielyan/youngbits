<div class="table-responsive">
    <table class="table table-striped">
        ...
        <tr>
            ...
            <th><a <?= orderableTH($this->input->get(), 'quote_status_id', 'ip_quotes'); ?>><?php _trans('status'); ?></a></th>
            <th><a <?= orderableTH($this->input->get(), 'quote_number', 'ip_quotes'); ?>><?php _trans('quote'); ?></a></th>
            <th><a <?= orderableTH($this->input->get(), 'quote_date_created', 'ip_quotes'); ?>><?php _trans('created'); ?></a></th>
            <th><a <?= orderableTH($this->input->get(), 'quote_date_expires', 'ip_quotes'); ?>><?php _trans('due_date'); ?></a></th>
            <th><a <?= orderableTH($this->input->get(), 'client_name', 'ip_clients'); ?>><?php _trans('client_name'); ?></a></th>
            <th style="text-align: right; padding-right: 25px;"><a <?= orderableTH($this->input->get(), 'quote_total', 'ip_quote_amounts'); ?>><?php _trans('amount'); ?></th>
        ...
        foreach ($quotes as $quote) {
            ...
            ?>
            <tr>
                <td><input type="checkbox" value="<?= $quote->quote_id ?>" class="sel"></td>
                <td>
