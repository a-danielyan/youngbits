...
//            $quote_total = $quote_amount->quote_item_subtotal + $quote_amount->quote_item_tax_total + $quote_amount->quote_tax_total;
    ...
    public function get_status_totals_all(){

        $results = $this->mdl_quotes->get()->result();


        foreach ($results as $res){
            if ($res->quote_status_id == 1) {
                $this->mdl_quotes->is_draft()->get()->result();
                $status = 'draft';
            }elseif ($res->quote_status_id == 2 ) {
                $this->mdl_quotes->is_sent();
                $status = 'sent';
            }elseif ($res->quote_status_id == 3) {
                $this->mdl_quotes->is_viewed();
                $status = 'viewed';
            }elseif ($res->quote_status_id == 4) {
                $this->mdl_quotes->is_paid();
                $status = 'paid';
            }elseif ($res->quote_status_id == 5) {
                $this->mdl_quotes->is_overdue();
                $status = 'overdue';
            }else{
                $status = 'all';
            }
            $res->quote_status_id  = $status;
