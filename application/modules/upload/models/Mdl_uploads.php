...
class Mdl_Uploads extends Response_Model
{
     ...
     * @return array
     */
    public function get_offer_uploads($id)
    {
        $this->load->model('offers/mdl_offers');
        $offer = $this->mdl_offers->get_by_id($id);
        $query = $this->db->query("SELECT file_name_new,file_name_original FROM ip_uploads WHERE url_key = '" . $offer->offer_url_key . "'");
