...
class Settings extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->post('settings')) {
            ...
            // Check for offer logo upload
            if (!empty($_FILES['offer_logo']) && $_FILES['offer_logo']['name']) {
                $this->load->library('upload', $upload_config);
                ...
                $this->mdl_settings->save('offer_logo', $upload_data['file_name']);
            }
            ...
            // Check for login bottom logo upload
            if ($_FILES['login_bottom_logo']['name']) {
                $this->load->library('upload', $upload_config);

                if (!$this->upload->do_upload('login_bottom_logo')) {
                    $this->session->set_flashdata('alert_error', $this->upload->display_errors());
                    redirect('settings');
                }
                ...
                $this->mdl_settings->save('login_bottom_logo', $upload_data['file_name']);
            }
        ...
        $pdf_offer_templates = $this->mdl_templates->get_offer_templates('pdf');
        $public_offer_templates = $this->mdl_templates->get_offer_templates('public');
...
//        $this->session->set_userdata("statuses", $status);
        ...
        $this->layout->set(
            array(
                ...
                'public_offer_templates' => $public_offer_templates,
                'pdf_invoice_templates' => $pdf_invoice_templates,
                'pdf_offer_templates' => $pdf_offer_templates,
