 ...
 * Class Mdl_expenses
 */
class Mdl_expenses_templates extends Response_Model
{
    public $table = 'ip_expenses_templates';
    public $primary_key = 'ip_expenses_templates.expenses_id';

    public function default_select()
    {
        $this->db->select("SQL_CALC_FOUND_ROWS ip_expenses_templates.*,ip_projects.*,ip_users.user_name", false);
    }
    ...
    public function default_order_by()
    {
        ...
        $orderTable = $this->input->get('table') ? ($this->input->get('table') === 'INDEPENDENT' ? '' : $this->input->get('table').'.') : 'ip_expenses_templates.';
        $this->db->order_by($orderTable.$orderField.' '.$orderType);
    }

    public function default_join()
    {
        $this->db->join('ip_projects', 'ip_projects.project_id = ip_expenses_templates.expenses_project_id', 'left');
        $this->db->join('ip_users', 'ip_users.user_id = ip_expenses_templates.expenses_user_id', 'left');
    ...
    public function db_array()
    {
        ...
        $db_array['expenses_date'] = date_to_mysql($db_array['expenses_date']);
