...
<link rel="stylesheet" href="<?php echo base_url(); ?>assets/core/js/dropzone.js">
...
<?php if (get_setting('monospace_amounts') == 1) { ?>
    <link rel="stylesheet"
          href="<?php echo base_url(); ?>assets/<?php echo get_setting('system_theme', 'Spudu'); ?>/css/monospace.css">
<?php } ?>
...
{?>
        $(document).on('click', '.create-offer', function () {
            $('#modal-placeholder').load("<?php echo site_url('offers/ajax/modal_create_offer'); ?>");
        });
<?php
}
?>
        ...
        $(document).on('click', '.client-create-offer', function () {
            var client_id = $(this).data('client-id');
            $('#modal-placeholder').load("<?php echo site_url('offers/ajax/modal_create_offer'); ?>", {client_id: client_id});
        });
