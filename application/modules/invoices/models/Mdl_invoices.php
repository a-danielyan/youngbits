...
class Mdl_Invoices extends Response_Model
{
    ...
    public function statuses()
    {
        return array(
            ...
            '5' => array(
                ...
                'href' => 'invoices/status/credit_invoice'
            )
        );
    }
    ...
    public function default_order_by()
    {
        ...
        $ip_invoices = $this->input->get('table') ? $this->input->get('table') : 'ip_invoices';
    ...
    public function default_join()
    {
        ...
        $this->db->join('ip_invoice_currency', 'ip_invoice_currency.invoice_id = ip_invoices.invoice_id', 'left');
    ...
    public function copy_invoice($source_id, $target_id, $copy_recurring_items_only = false)
    {
        ...
        foreach ($invoice_items as $invoice_item) {
            $db_array = array(
                ...
                'item_product_SKU' => $invoice_item->item_product_SKU,
...
//     var_dump($db_array['invoice_terms']);die;
    ...
    {
        $this->filter_where_in('invoice_status_id', array(2, 3, 4, 5));
        return $this;
    }
    ...
    public function send_msg($invoice_id){

        $this->db->where('invoice_id', $invoice_id);
            $this->db->set('invoice_last_msg_date_stamp', date('Y-m-d h:i:s'));
        $this->db->update('ip_invoices');
    }
    ...
    public function all_invoices(){
        ...
        //$sql = "select * from ip_invoices  JOIN ip_invoice_amounts ON ip_invoice_amounts.invoice_id =ip_invoices.invoice_id";

        //$query = $this->db->query($sql);
        return  $this->mdl_invoices->join('ip_invoice_amounts','ip_invoice_amounts.invoice_id =ip_invoices.invoice_id')->result();

    }

    public function get_years($order = 'desc') {
        $this->db->select('YEAR(invoice_date_created) as year');
    ...
    public function compbyning_by_year($years, $table = '', $amount = '')
    {
        ...
        // here Invoices is a category
        //  we have 3 categories Invoices, expenses euro, expenses dollar
        // get all 3 and marge in controller
        $all_years_quarters = [];
        $for_expenses = false;

        if ($table == 'mdl_expenses') {
            $for_expenses = true;
        }


        // all quarters data for Invoices
        foreach ($years as $key => $year) {
            $all_years_quarters['1st quarter '.$year['year']] = [];
            $all_years_quarters['1st quarter '.$year['year']]['total_amount_with_taxes']    = $for_expenses ? $this->column_summary_for_expenses(1, 'expenses_amount', $amount, $year['year']) : $this->column_summary(1, 'invoice_total', $year['year']);
            $all_years_quarters['1st quarter '.$year['year']]['total_taxes']                = $for_expenses ? $this->column_summary_for_expenses(1, 'expenses_taxes', $amount, $year['year']) : $this->column_summary(1, 'invoice_item_tax_total', $year['year']);
            $all_years_quarters['1st quarter '.$year['year']]['total_amount_without_taxes'] = $for_expenses ? ($all_years_quarters['1st quarter '.$year['year']]['total_amount_with_taxes'] - $all_years_quarters['1st quarter '.$year['year']]['total_taxes']) : $this->column_summary(1, 'invoice_item_subtotal', $year['year']);
            $all_years_quarters['1st quarter '.$year['year']]['category']                   = $for_expenses && $amount ? $amount.'_expenses' : 'Invoices';



            $all_years_quarters['2nd quarter '.$year['year']] = [];
            $all_years_quarters['2nd quarter '.$year['year']]['total_amount_with_taxes']    = $for_expenses ? $this->column_summary_for_expenses(2, 'expenses_amount', $amount, $year['year']) : $this->column_summary(2, 'invoice_total', $year['year']);
            $all_years_quarters['2nd quarter '.$year['year']]['total_taxes']                = $for_expenses ? $this->column_summary_for_expenses(2, 'expenses_taxes', $amount, $year['year']) : $this->column_summary(2, 'invoice_item_tax_total', $year['year']);
            $all_years_quarters['2nd quarter '.$year['year']]['total_amount_without_taxes'] = $for_expenses ? ($all_years_quarters['2nd quarter '.$year['year']]['total_amount_with_taxes'] - $all_years_quarters['2nd quarter '.$year['year']]['total_taxes']) : $this->column_summary(2, 'invoice_item_subtotal', $year['year']);
            $all_years_quarters['2nd quarter '.$year['year']]['category']                   = $for_expenses && $amount ? $amount.'_expenses' : 'Invoices';



            $all_years_quarters['3th quarter '.$year['year']] = [];
            $all_years_quarters['3th quarter '.$year['year']]['total_amount_with_taxes']    = $for_expenses ? $this->column_summary_for_expenses(3, 'expenses_amount', $amount, $year['year']) : $this->column_summary(3, 'invoice_total', $year['year']);
            $all_years_quarters['3th quarter '.$year['year']]['total_taxes']                = $for_expenses ? $this->column_summary_for_expenses(3, 'expenses_taxes', $amount, $year['year']) : $this->column_summary(3, 'invoice_item_tax_total', $year['year']);
            $all_years_quarters['3th quarter '.$year['year']]['total_amount_without_taxes'] = $for_expenses ? ($all_years_quarters['3th quarter '.$year['year']]['total_amount_with_taxes'] - $all_years_quarters['3th quarter '.$year['year']]['total_taxes']) : $this->column_summary(3, 'invoice_item_subtotal', $year['year']);
            $all_years_quarters['3th quarter '.$year['year']]['category']                   = $for_expenses && $amount ? $amount.'_expenses' : 'Invoices';


            $all_years_quarters['4th quarter '.$year['year']] = [];
            $all_years_quarters['4th quarter '.$year['year']]['total_amount_with_taxes']    = $for_expenses ? $this->column_summary_for_expenses(4, 'expenses_amount', $amount, $year['year']) : $this->column_summary(4, 'invoice_total', $year['year']);
            $all_years_quarters['4th quarter '.$year['year']]['total_taxes']                = $for_expenses ? $this->column_summary_for_expenses(4, 'expenses_taxes', $amount, $year['year']) : $this->column_summary(4, 'invoice_item_tax_total', $year['year']);
            $all_years_quarters['4th quarter '.$year['year']]['total_amount_without_taxes'] = $for_expenses ? ($all_years_quarters['4th quarter '.$year['year']]['total_amount_with_taxes'] - $all_years_quarters['4th quarter '.$year['year']]['total_taxes'] ) : $this->column_summary(4, 'invoice_item_subtotal', $year['year']);
            $all_years_quarters['4th quarter '.$year['year']]['category']                   = $for_expenses && $amount ? $amount.'_expenses' : 'Invoices';
        }
        return $all_years_quarters;
    }

    private function column_summary($quarter, $column, $year)
    {
        $month = $this->get_months_by_quarter ($quarter);
        // if (date('Y') == $year && (date('m') < $month[count($month)-1] && !in_array(date('m'), $month))) {
        //     return null;
        // }

        $this->db->select('SUM(ip_invoice_amounts.' . $column . ') AS ' . $column);
        $this->db->from('ip_invoices');
        ...
        $this->db->where_in('MONTH(invoice_date_created)', $month);
   ...
   private function column_summary_for_expenses($quarter, $column, $amount, $year, $currency = 'dollar')
   {
        // require_once __DIR__.'\..\..\expenses\models\Mdl_expenses.php';
        // $mdl_expenses = new Mdl_expenses;
        $month = $this->get_months_by_quarter ($quarter);
        // if (date('Y') == $year && (date('m') < $month[count($month)-1] && !in_array(date('m'), $month))) {
        //     return null;
        // }
        if ($amount == 'euro' && $column == 'expenses_amount') {
            $column .= '_'.$amount;
            $currency = 'euro';
        }
        $column_name = 'total_amount_in_'.$amount;

        $this->db->select('SUM(' . $column . ') AS '.$column_name);
        $this->db->from('ip_expenses');
        $this->db->where('expenses_currency', $currency);
        $this->db->where('YEAR(expenses_date)', $year);
        $this->db->where_in('MONTH(expenses_date)', $month);
        $result = $this->db->get()->row();
   ...
   public function calculate_quarter_amount($quarter, $column, $total = 0){
