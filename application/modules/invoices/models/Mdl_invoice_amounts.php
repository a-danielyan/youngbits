...
//        $this->calculate_invoice_taxes($invoice_id);
    ...
    public function calculate_discount($invoice_id, $invoice_total)
    {
        ...
        $total = $total + $invoice_data->offer_transport + $invoice_data->offer_transport_tax;
        ...
        if ($invoice_tax_rates) {
            ...
            if (!empty($invoice_amount)) {
                foreach ($invoice_tax_rates as $invoice_tax_rate) {
                    ...
                    $db_array = array(
                        'invoice_tax_rate_amount' => ($invoice_tax_rate_amount != null) ? $invoice_tax_rate_amount : 0
                    );
    ...
    public function convert_amount($from_currency , $to_currency, $invoice_id, $has_currency = null)
    {
        if( $from_currency == $to_currency )
            return;

        $invoice_items = $this->db->from('ip_invoice_items')
            ->where('invoice_id', $invoice_id)
            ->get()->result();
        $idItems = array_column($invoice_items, 'item_id');

        $to_currency_rate = get_converted_currency_rate( $from_currency,$to_currency);
        ...
        if( !$has_currency ){
            $this->db->insert('ip_invoice_currency', $db_array);
        }else{
            $this->db->where('invoice_id', $invoice_id);
            $this->db->update('ip_invoice_currency', $db_array);
        }
