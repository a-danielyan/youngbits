    ...
    $(function () {
        ...
        $('#btn_modal_payment_submit').click(function () {
            $.post("<?php echo site_url('payments/ajax/add'); ?>", {
                    ...
                    subscriptions_date: $('#subscriptions_date').val(),
                    subscriptions_start_date: $('#subscriptions_start_date').val(),
                    subscriptions_amount: $('#subscriptions_amount').val(),
                    subscriptions_end_date: $('#subscriptions_end_date').val(),
                    subscriptions_frequency: $('#subscriptions_frequency').val(),
                    subscriptions_contact: $('#subscriptions_contact').val(),
                    subscriptions_username: $('#subscriptions_username').val(),
                    subscriptions_note: $('#subscriptions_note').val()
                },
                function (data) {
                    <?php echo(IP_DEBUG ? 'console.log(data);' : ''); ?>
                    var response = JSON.parse(data);
                    if (response.success === 1) {
                        // The validation was successful and subscriptions was added
